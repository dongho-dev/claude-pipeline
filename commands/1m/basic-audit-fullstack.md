> 풀스택 감사 → 이슈 생성 | context_tier: 1m

백엔드 + 프론트엔드 코드를 동시에 감사하여 개선/수정이 필요한 항목을 발견하고, 이슈로 등록합니다.

인자: `$ARGUMENTS` (선택, 집중 영역 지정 예: `security`, `error-handling`, `ux`. 없으면 전체 관심사 실행)

## Step 1: 기존 이슈 수집 + 라벨 사전체크

```bash
gh issue list --state open --limit 200 --json number,title,labels,body
gh label list --json name
```

1. 이슈 목록(body 포함)을 이후 모든 발견과 대조한다. 이미 이슈가 있는 문제는 건너뛴다. body의 파일:라인까지 비교하여 정확하게 대조한다.
2. Step 4에서 사용할 라벨(`security`, `bug`, `enhancement`, `performance`, `test`, `refactor` 등)이 존재하는지 확인한다. 없는 라벨은 `gh label create`로 미리 생성한다.

## Step 2: 관심사별 Explore Agent 병렬 실행

7개 관심사 × 관심사당 1개 Explore Agent(model: **sonnet**)를 **병렬로** 실행한다. `$ARGUMENTS`로 영역이 지정된 경우 해당 agent만 실행한다 (예: `security` → Agent B1 + F1만, `hygiene` → Agent C1만).

**agent 수 조정:** 실행 전 관심사별 대상 파일의 총 크기를 `wc -c`로 측정한다. **2MB당 1 agent** (올림)를 기준으로, 2MB 초과 시 해당 관심사를 디렉토리별로 분할하여 복수 agent를 실행한다.

### Backend Agents

#### Agent B1: 보안 + 입력 검증

프로젝트에서 API 라우트, 인증 미들웨어, 입력 검증 관련 코드를 스스로 탐색하여 보안 취약점과 입력 검증 누락을 찾는다. 코드를 직접 읽고 문제를 특정하라.

#### Agent B2: 에러 핸들링 + API 일관성

프로젝트에서 API 라우트, 서버사이드 로직, 데이터 레이어 코드를 스스로 탐색하여 에러 처리 누락, 응답 형식 불일치, 장애 대응 부재를 찾는다. 코드를 직접 읽고 문제를 특정하라.

#### Agent B3: 테스트 커버리지 + 데이터 무결성

프로젝트에서 테스트 파일, 데이터 레이어, DB 스키마를 스스로 탐색하여 테스트 누락, 타입-스키마 불일치, 데이터 처리 오류를 찾는다. 코드를 직접 읽고 문제를 특정하라.

### Frontend Agents

#### Agent F1: 보안 + 데이터 무결성

프로젝트에서 페이지 컴포넌트, UI 컴포넌트, 커스텀 훅, 클라이언트 사이드 유틸을 스스로 탐색하여 클라이언트 사이드 보안 취약점과 민감 데이터 노출을 찾는다. 코드를 직접 읽고 문제를 특정하라.

#### Agent F2: UX 일관성 + 접근성

프로젝트에서 페이지 컴포넌트, UI 컴포넌트, 로딩/에러 상태 컴포넌트를 스스로 탐색하여 UX 불일치, 접근성 누락, 사용자 피드백 부재를 찾는다. 코드를 직접 읽고 문제를 특정하라.

#### Agent F3: 성능 + 코드 품질

프로젝트에서 UI 컴포넌트, 커스텀 훅, 클라이언트 유틸을 스스로 탐색하여 불필요한 리렌더, 번들 비대화, 테스트 부재, 타입 안전성 문제를 찾는다. 코드를 직접 읽고 문제를 특정하라.

### Cross-layer Agent

#### Agent C1: 코드 위생

프로젝트의 백엔드 + 프론트엔드 소스 코드 전반을 탐색하여 코드 위생 문제를 찾는다. 코드를 직접 읽고 문제를 특정하라.

탐색 대상:
- 미사용 import / 미사용 변수
- 도달 불가 코드, 호출되지 않는 export (dead code)
- 남겨진 console.log / debugger / 주석 처리된 코드 블록
- TODO/FIXME/HACK 주석 중 실제 작업이 필요한 것
- 네이밍 불일치 (camelCase/snake_case 혼용 등)

### Agent 공통 지시

각 Agent에게 전달할 지시:
```
UI 라이브러리 프리미티브(shadcn 등)는 감사하지 않는다.

발견 항목을 아래 형식으로 반환하라:

### 발견 1: [문제 요약]
- 위치: `파일:라인`
- 영역: backend / frontend
- 근거: 확인됨 / 추정
- 코드:
  ```
  // 해당 라인 주변 3-5줄 스니펫
  ```
- 설명: 구체적으로 무엇이 왜 문제인지

### 발견 2: ...

규칙:
- 근거 "확인됨": 코드를 직접 읽고 문제를 특정한 경우 (파일:라인 필수)
- 근거 "추정": 코드 패턴상 문제가 있을 것 같지만 완전히 확인하지 못한 경우
- 추정이라도 의심되면 보고한다 (본체가 2차 판단)
- UI 라이브러리 프리미티브(shadcn 등)는 감사하지 않는다
- 이미 해결된 것은 보고하지 않는다
```

## Step 3: 발견 통합 + 이슈 그룹핑

7개 Agent 결과를 수집하여:

1. **중복 제거** — 같은 파일:라인이 여러 agent에서 보고된 경우 병합
2. **크로스 레이어 발견** — 백엔드/프론트엔드 양쪽에서 관련된 문제(예: API 응답 형식 ↔ 프론트 파싱 불일치)는 하나의 이슈로 묶는다
3. **기존 이슈 대조** — Step 1에서 수집한 open 이슈와 비교 (제목 + body의 파일:라인), 이미 있으면 스킵
4. **그룹핑 판단**:
   - 같은 패턴의 반복 → 하나의 이슈로 그룹
   - 고유한 문제 → 단독 이슈
5. **자동 필터링 — 이슈 생성 대상 결정**:
   - 근거 "확인됨" + 기존 이슈와 중복 아님 → **자동 생성**
   - 근거 "추정" → 본체가 해당 파일:라인을 직접 Read로 읽고 2차 판단 → 진짜면 생성, 아니면 버림
   - 기존 이슈와 겹침 → **생성하지 않음** (스킵 로그에 기록)

## Step 4: 이슈 생성

필터링을 통과한 항목에 대해 **Bash 병렬 호출**로 이슈를 일괄 생성한다.
에이전트를 띄우지 않고 본체가 직접 `gh issue create`를 병렬 Bash tool call로 실행한다.

```bash
gh issue create \
  --title "제목" \
  --body "$(cat <<'EOF'
## 문제

[감사에서 발견한 내용]

## 해당 위치

- `파일:라인` — 구체적 문제
- ...

## 제안

[수정 방향]

---
🤖 `/basic-audit-fullstack`에서 자동 생성됨
EOF
)" \
  --label "라벨"
```

라벨 매핑:
- 보안 관련 → `security`
- 에러 핸들링 → `bug` 또는 `refactor`
- 테스트 부재 → `test`
- API 일관성 → `refactor`
- UX/접근성 → `enhancement`
- 성능 → `performance`
- 코드 위생 → `hygiene`

**실행 방식:** 이슈별로 독립된 Bash tool call을 만들어 한 메시지에서 병렬 실행. 에이전트 오버헤드 없이 빠르게 생성.

## Step 5: 결과 요약

```
## 감사 완료

- 탐색 영역: fullstack (backend + frontend)
- 발견 항목: N개
  - backend: B개 (보안 X / 에러핸들링 Y / 테스트 Z)
  - frontend: F개 (보안 X / UX Y / 성능 Z)
  - cross-layer: C개
  - 코드위생: H개
- 생성 이슈: M개
- 스킵 (기존 이슈): K개
- 추정 → 버림: L개

| 이슈 | 제목 | 영역 | 관심사 | 라벨 |
|------|------|------|--------|------|
| #401 | ... | backend | 보안 | security |
| #402 | ... | frontend | UX | enhancement |
| #403 | ... | cross-layer | API↔파싱 | refactor |

### 스킵 (기존 이슈와 중복)
| 발견 | 기존 이슈 | 이유 |
|------|----------|------|
| ... | #N | ... |

### 오탐 판정
| 발견 | 판정 이유 |
|------|----------|
| ... | ... |

다음 단계: `/spec 401 402 403` 으로 스펙 생성
```
