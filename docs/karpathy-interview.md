...
안드레 카파시는 업계에서도 아주 빠르게 openclaw, moltbook를 직접 사용하면서 최근에는 auto-research(https://github.com/karpathy/autoresearch)를 내놓아 AI 권위자 중에서 아주 트렌디쉬한 면모를 보입니다.
...
안드레 카파시: 알겠습니다.
사라 궈: 트위터 @nopriorspod에서 저희를 찾아주세요. 저희 얼굴을 보고 싶으시다면 유튜브 채널을 구독해 주세요. Apple Podcasts, Spotify 또는 즐겨 들으시는 팟캐스트 플랫폼에서 저희 쇼를 팔로우하세요. 그러면 매주 새로운 에피소드를 받아보실 수 있습니다. 그리고 no-priors.com에서 이메일 뉴스레터를 신청하시거나 모든 에피소드의 대본을 확인해 보세요.
[각주 및 전문 용어 설명]
Claude Code / Codex: Anthropic과 OpenAI에서 각각 개발한 코딩 특화 AI 모델 및 에이전트 도구입니다.
에이전트 하네스(agent harnesses): 에이전트를 실행, 제어, 평가하기 위해 감싸는 뼈대나 인터페이스 환경을 뜻합니다.
No Priors: Sarah Guo와 Elad Gil이 진행하는 AI 팟캐스트입니다. 
OpenClaw: Peter Steinberger가 개발한 오픈 소스 자율 AI 에이전트로, 메시징 인터페이스 등을 통해 백그라운드에서 지속적으로 작동합니다.
피터 스타인버거(Peter Steinberger): 개발자이자 OpenClaw 프로젝트를 만든 인물로, 다수의 에이전트를 조율하는 방식으로 화제가 되었습니다. 
SOUL.md 문서: 에이전트의 페르소나와 성격을 정의하기 위해 사용하는 마크다운 문서입니다. 
 도비(Dobby the Elf): 해리 포터 시리즈의 집요정 이름에서 따온, 카파시가 자신의 홈 오토메이션 에이전트에 붙인 이름입니다.
Qwen: 알리바바(Alibaba) 그룹이 개발한 거대 언어 모델(Vision 모델 포함)입니다. 
오토리서치(AutoResearch): 안드레 카파시가 공개한 자율 머신러닝 연구 시스템으로, AI가 실험을 통해 모델 파라미터를 스스로 튜닝합니다.
nanochat: 카파시가 LLM 훈련을 실험하기 위해 구축한 초경량화된 학습 환경 및 모델입니다. 
바이브 코딩(vibe coding): 명확한 문법의 코드를 직접 짜기보다는 에이전트에게 느낌이나 의도만 자연어로 전달해 코드를 짜게 하는 방식을 의미합니다.
가중치 감쇠(weight decay) 및 Adam beta: 딥러닝 모델의 최적화 알고리즘(Adam)에서 과적합을 막고 학습 속도를 조절하는 핵심 하이퍼파라미터입니다. 
program.md: 오토리서치 에이전트가 어떤 방식으로 연구를 수행할지에 대한 지침을 마크다운으로 정의한 파일입니다.
들쭉날쭉함(jaggedness): 어떤 분야에서는 엄청난 지능을 보이다가도 기초적인 상식이나 추론에서는 매우 멍청한 실수를 하는 AI의 불균일한 지능 상태를 뜻합니다.
...
에이전트가 실서비스로 나가기 위해 가장 시급한 건 더 똑똑한 모델이 아니라, 바로 이런 물리적인 안전 계층입니다.