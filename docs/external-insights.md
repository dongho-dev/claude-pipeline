...
## "Why" 에이전트 — 관점 다양성 기반 품질 게이트 (아이디어)

### 문제 인식

현재 파이프라인의 모든 에이전트가 **How/What** 역할만 수행:
- Audit → "뭐가 문제인가?" (발견)
- Spec → "뭘 바꿀 것인가?" (계획)
- Worker → "어떻게 바꾸는가?" (실행)
- Reviewer → "제대로 바꿨는가?" (검증)

**"Why"를 묻는 역할이 없음.** 이슈가 정말 필요한지, 이 접근이 최선인지, 뭘 깨뜨릴 수 있는지를 사전에 도전하는 에이전트가 부재.

### 후보 역할

| 역할 | 질문 | 위치 | 효과 |
|------|------|------|------|
| **Devil's Advocate** | "이거 정말 필요해? 안 하면 어떻게 되는데?" | nextplan 후 | 불필요한 이슈 필터링 |
| **소크라테스** | "왜 이 방식이지? 더 단순한 방법은?" | spec 후 | 과잉 엔지니어링 방지 |
| **리스크** | "이거 깨뜨릴 수 있는 건 뭐지?" | spec 후 | 사전 리스크 식별 |

### 근거

**1. 앙상블 관점 다양성 (Part 3 앙상블 원칙)**
같은 모델이라도 프롬프트가 다르면 다른 오류 패턴을 가짐. 생성-검증 루프(Worker→Reviewer)는 있지만, **계획 단계의 검증**이 없음. ICLR 2025 데이터에서 멀티에이전트 실패의 37%가 명세/시스템 설계 — 계획 단계에서 도전하는 에이전트가 이 37%를 줄일 수 있음.

**2. Ouroboros 소크라테스 인터뷰 (1C)**
Ouroboros가 모호성 스코어링으로 명세 품질을 강제한 것의 경량 버전. 전체 인터뷰 엔진 대신 "왜?"라는 단일 질문으로 동일 효과를 노림.

**3. Karpathy "70% 문제 정의, 30% 실행"**
현재 파이프라인은 실행(Worker+Reviewer)에 에이전트 자원이 집중. 문제 정의(nextplan+spec)에 "도전자" 에이전트를 추가하면 Karpathy 원칙에 더 부합.

**4. Cognition "멀티에이전트 = 충돌하는 결정"에 대한 반론**
Cognition이 비판한 것은 **같은 역할의 에이전트가 같은 자원에서 충돌**하는 것. 다른 관점(생성 vs 도전)의 에이전트는 충돌이 아니라 **보완**. Cognition 자체도 Planner+Coder+Critic+Browser 4역할을 사용.

**5. 소크라테스식 학습 프롬프트의 확장**
학습 도메인에서 검증된 "정답을 주지 말고 질문만 던져라" 패턴을 코드 품질 도메인에 전이. 학습에서 효과적인 것은 의사결정에서도 효과적.

### 트레이드오프

| 관점 | 도입 | 미도입 |
|------|------|--------|
| **비용** | 이슈당 Sonnet 1회 추가 | 0 |
| **속도** | phase 1개 추가 → 파이프라인 시간 증가 | 현행 유지 |
| **효과** | 불필요한 이슈/과잉 구현 사전 차단 | Reviewer에서 사후 발견 |
| **리스크** | "Why" 에이전트가 모든 이슈를 과도하게 차단할 수 있음 | 없음 |

### 적용 방안 (단계적)

**Phase 1: spec 프롬프트 내 자기 도전** (비용 0)
- spec 에이전트에게 "명세 작성 후, 스스로 다음을 물어라: 이 변경이 정말 필요한가? 더 단순한 방법은 없는가? 뭘 깨뜨릴 수 있는가?" 지시 추가
- 별도 에이전트 없이 자기 반성으로 시작

**Phase 2: 독립 에이전트 분리** (Sonnet 비용)
- `/basic-challenge` 커맨드 — spec 결과를 받아 도전 질문 생성
- 답변이 불충분하면 spec 재작성 트리거
- `priority:high` 이슈에만 선택적 적용

**Phase 3: Paperclip 조직 내 역할화**
- Paperclip 조직에 "Chief Skeptic" 에이전트 등록
- 모든 spec을 자동으로 도전, 승인 게이트 역할

### 우선순위: 낮 → spec 부실 → Worker FAIL 패턴이 반복될 때 재평가
- Phase 1(자기 도전)은 프롬프트 1-2줄이라 즉시 가능하지만, 효과 측정이 어려움
- 실패 원인 분석(agent-blame)에서 "spec 부실"이 주요 원인으로 나타나면 Phase 2 도입
